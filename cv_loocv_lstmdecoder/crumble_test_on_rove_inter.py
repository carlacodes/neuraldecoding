...
def target_vs_probe(blocks, talker=1, probewords=[20, 22], pitchshift=True, window=[0, 0.5]):
    ...
    for cluster_id in tqdm(clust_ids):
        ...
        # try:
        if talker == 1:
            X_high, y_high, unique_trials_probe_high, unique_trials_targ_high = generate_talker_raster_for_lstm(blocks,
                                                                                                                cluster_id,
                                                                                                                window,
                                                                                                                binsize,
                                                                                                                probeword=probeword,
            ...
            X_high, y_high, unique_trials_probe_high, unique_trials_targ_high = generate_talker_raster_for_lstm(
                blocks, cluster_id, window, binsize, probeword=probeword, talker_choice=8, control_pitch=False)
            X_low, y_low, unique_trials_probe_low, unique_trials_targ_low = generate_talker_raster_for_lstm(blocks,
                                                                                                            cluster_id,
                                                                                                            window,
                                                                                                            binsize,
...
def run_classification(datapath, ferretid, ferretid_fancy='F1902_Eclair'):
    ...
    tarDir = Path(
        f'E:/test_results_testonrove_inter_28102023/{ferretid_fancy}/{recname}/{stream_used}/')
    saveDir = tarDir
    ...
    for probeword in probewords_list:
        ...
        for talker in [1, 2]:
            ...
            scores[f'talker{talker}']['target_vs_probe']['nopitchshiftvspitchshift'] = target_vs_probe(blocks,
                                                                                                       talker=talker,
...
def main():
    datapath_big = Path(f'E:\ms4output2\F1901_Crumble/')
