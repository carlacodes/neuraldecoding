...
def target_vs_probe(blocks, talker=1, probewords=[20, 22], pitchshift=True):
    ...
    for cluster_id in tqdm(clust_ids):
        ...
        scores['score'].append(score)
        scores['lstm_score'].append(np.mean(totalaclist))
        scores['lstm_balanced_avg'].append(np.mean(outsideloopacclist))
        scores['bootScore'].append(bootScore)
        scores['lstm_accuracylist'].append(outsideloopacclist)
        scores['lstm_avg'].append(np.mean(outsideloopacclist))
...
def run_classification(dir, datapath, ferretid):
    ...
    tarDir = Path(f'E:/decoding_scores/F2003_Orecchiette\lstm_kfold_20062023_ores2/')
    saveDir = tarDir
...
def main():
    directories = ['orecchiette_2022']  # , 'Trifle_July_2022']/home/zceccgr/Scratch/zceccgr/ms4output/F1702_Zola/spkenvresults04102022allrowsbut4th
