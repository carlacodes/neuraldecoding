...
import sklearn
from instruments.helpers.neural_analysis_helpers import get_word_aligned_raster_with_pitchshift
import numpy as np
import pickle
...
def target_vs_probe(blocks, talker=1, probewords=[20, 22], pitchshift=True, window=[0, 0.5]):
    ...
    for cluster_id in tqdm(clust_ids):
        ...
        # try:
        if talker == 1:
            X_high, y_high, unique_trials_probe_high, unique_trials_targ_high = generate_talker_raster_for_lstm(blocks, cluster_id, window, binsize, probeword = probeword, talker_choice=5)
            X_low, y_low, unique_trials_probe_low, unique_trials_targ_low = generate_talker_raster_for_lstm(blocks, cluster_id, window, binsize, probeword = probeword, talker_choice=3)
            X, y, unique_trials_probe, unique_trials_targ = generate_talker_raster_for_lstm(blocks, cluster_id, window,
                                                                                        binsize, probeword=probeword,
                                                                                        talker_choice=1)
            ...
            X_low, y_low, unique_trials_probe_low, unique_trials_targ_low = generate_talker_raster_for_lstm(blocks,
                                                                                                            ...
                                                                                                            binsize,
                                                                                                                probeword=probeword,
                                                                                                              talker_choice=13)
        if len(X) == 0 or len(X_high) == 0 or len(X_low) == 0:
...
def run_classification(datapath, ferretid, ferretid_fancy='F1902_Eclair'):
    ...
    tarDir = Path(
        f'F:/lstmdecodingproject/leavepoutcrossvalidationlstmdecoder/results_testonrove_intra_27102023/{ferretid_fancy}/{recname}/{stream_used}/')
    saveDir = tarDir
