...
from instruments.helpers.analysis_helpers import apply_filter
...
def get_word_aligned_raster_ore(blocks, clust_id, word=None, pitchshift=True, correctresp=True, df_filter=[],
    ...
    for s, seg in enumerate(blocks[0].segments):
        ...
        if 'PitchShiftMat' in seg.annotations['bhv_data']:
            print('This is an intra-trial pitch shift level, skipping iteration..' + str(s))
            # print(f"Block name: {seg.df_bhv['recBlock']}, iteration:" + str(s))
            continue
...
def get_word_aligned_raster_inter_by_pitch(blocks, clust_id, word=None, talker_choice=1, correctresp=True,
                                           df_filter=[]):
    unit_aligned_time = np.array([])
    unit_aligned_time_compare = np.array([])
    for s, seg in enumerate(blocks[0].segments):
        ...
        if word is None:
            word = seg.df_bhv.probes[0]
        df_bhv = seg.df_bhv
        df_bhv = df_bhv.loc[df_bhv.talker == talker_choice]
...
def get_word_aligned_raster_squinty_bypitch(blocks, clust_id, word=None, talker_choice=1, correctresp=True,
                                            df_filter=[]):
    unit_aligned_time = np.array([])
    for s, seg in enumerate(blocks[0].segments):
        if seg.df_bhv is None:
...
#
def get_word_aligned_raster(blocks, clust_id, word=None, pitchshift=True, correctresp=True, df_filter=[]):
    unit_aligned_time = np.array([])
    for s, seg in enumerate(blocks[0].segments):
        if seg.df_bhv is None:
            continue
...
def get_soundonset_alignedraster_tabular(blocks, clust_id, df_filter=[]):
    all_spike_times = []  # List to hold spike times arrays for each trial
    trial_info_list = []  # List to hold trial metadata dictionaries

    # Iterate through each block and segment
    for block in blocks:
        for seg in block.segments:
            if seg.df_bhv is None:
                continue

            df_bhv = seg.df_bhv.copy()

            # Calculate trial timing columns
            df_bhv['targTimes'] = df_bhv['timeToTarget'] / 24414.0625
            df_bhv['centreRelease'] = df_bhv['lickRelease'] - df_bhv['startTrialLick']
            df_bhv['relReleaseTimes'] = df_bhv['centreRelease'] - df_bhv['targTimes']
            df_bhv['realRelReleaseTimes'] = df_bhv['relReleaseTimes'] - df_bhv['absentTime']
            ...
            # Get matching spiketrains for the cluster
            spiketrains = [st for st in seg.spiketrains
            ...
            for _, trial in df_bhv.iterrows():
                # Get trial time boundaries
                trial_start = trial['startTrialLick']
                trial_end = trial['lickRelease']

                # Extract spikes occurring during this trial
                spikes = unit.times[(unit.times >= trial_start) & (unit.times <= trial_end)]
    ...
    # Create DataFrame from collected trial info
    df_trials = pd.DataFrame(trial_info_list)

    return np.array(all_spike_times, dtype=object), df_trials
...
def get_word_aligned_raster_with_pitchshift(blocks, clust_id, word=None, correctresponse=True, selectedpitch=2,
    ...
    unit_aligned_time = np.array([])
    for s, seg in enumerate(blocks[0].segments):
        if seg.df_bhv is None or hasattr(seg.df_bhv, 'PitchShiftMat') == False:
            continue
        pitchshiftlist = np.array([])
        droplist = np.array([])
        for k in range(0, len(seg.df_bhv)):
