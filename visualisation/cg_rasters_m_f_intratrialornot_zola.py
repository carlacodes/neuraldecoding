...
def target_vs_probe_with_raster(blocks, talker=1, probewords=[20, 22], pitchshift=True):
    ...
    for cluster_id in tqdm(clust_ids):
        ...
        plt.suptitle('Target firings for zola,  clus id '+ str(cluster_id)+'pitchshift = '+str(pitchshift)+'talker'+str(talker), fontsize = 20)
        plt.savefig('D:/Data/rasterplotsfromdecoding/zola/mandf/zolatarg2_clusterid'+str(cluster_id)+' probeword '+str(probeword)+' pitch '+str(pitchshift)+'talker'+str(talker)+'.png')
        #plt.show()
        ...
        plt.setp(ax, xlim=custom_xlim)
        plt.suptitle('Distractor firings for zola,  clus id '+ str(cluster_id)+' , pitchshift = '+str(pitchshift)+ 'probeword '+str(probeword)+'talker'+str(talker), fontsize = 20)



        plt.savefig('D:/Data/rasterplotsfromdecoding/zola/mandf/zoladist2_clusterid'+  str(cluster_id)+'probe'+str(probeword)+' , pitch '+str(pitchshift)+'talker'+str(talker)+'.png')
        #plt.show()
    return
