...
def collapse_scores(df_full_naive, df_full):
    rel_frac_list_naive = []
    ...
    return rel_frac_list_naive, bigconcatenatenaive_ps, bigconcatenatenaive_nonps, rel_frac_list_trained, bigconcatenatetrained_nonps, bigconcatenatetrained_ps
                                ...
                                try:
                                    if score_key == 'lstm_balanced_avg':
                                        ...
                                        sorted_df_of_scores = sorted_df_of_scores.append(
                                            {'probeword1': probeword1_input_text[0], 'pitchshift': cond,
                                             'cluster_id': clus,
                                             'score': np.percentile(scores[f'talker{talker}'][comp][cond][score_key][i], 97.5),
                                             'unit_type': unit_type, 'animal': fullid, 'stream': stream_id,
                                             'recname': recname,
                                             'clus_id_report': clus_id_report, 'tdt_electrode_num': tdt_position,
                                             'brain_area': brain_area}, ignore_index=True)
                            ...
                            if electrode_position_dict:
                                ...
                                if score_key == 'lstm_balanced_avg':
                                    ...
                                    sorted_df_of_scores = sorted_df_of_scores.append(
                                        {'probeword1': probeword1_input_text[0], 'pitchshift': cond,
                                         'cluster_id': clus,
                                         'score': np.percentile(scores[f'talker{talker}'][comp][cond][score_key][i][0], 97.5),
            ...
            for stream in report[animal]:
                ...
                if animal == 'F1606_Windolene' or 'F1604_Squinty':
                    df_instance = load_scores_and_filter(probewordlist_l74,
                                                         saveDir=f'G:/results_01072024//{animal}/{rec_name_unique}/{streamtext}/',
                    ...
                    # try:
                    df_instance = load_scores_and_filter(probewordlist,
                                                         saveDir=f'G:/results_01072024//{animal}/{rec_name_unique}/',
                                                         ferretname=animal_text,
                                                         singleunitlist=singleunitlist[animal][stream],
                                                         multiunitlist=multiunitlist[animal][stream],
                                                         noiselist=noiselist[animal][stream], stream=stream,
...
def plot_major_analysis(df_merged):
    ...
    df_merged = df_merged[df_merged['BrainArea'] != 'VC']
    ...
    plt.xlabel(None)
    handles, labels = ax.get_legend_handles_labels()
    #replace x tick labels with probe word labels
