...
def main():
    ...
    dp = Path('D:/F1702_Zola/spkenvresults04102022allrowsbut4th/')
    warpData = Path('E:\Electrophysiological_Data\F1702_Zola_Nellie\Zola2021')
    saveDir = Path('D:/Data/spkfigs/')
    saveDir.mkdir(parents=False, exist_ok=True)

    dataset = concatenatedWarpData(dp, warpData=warpData)
    dataset.load()
    dataset.create_summary_pdf(saveDir, title='summary_pdf_Zola_summer_smoothed')
